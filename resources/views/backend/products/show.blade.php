    ...
    <div class="col-12">
        ...
        <div class="page-title-box">
            ...
            <div class="row">
                ...
                <div class="col-md-6"><strong>Category :</strong> {{$product->category_name}}</div><br>
            </div>
            <div class="row">
            ...
            <hr>
             <div class="row mb-3">
                 @if($product->image)
                    <div class="col-md-6">
                        <strong>Product Image :</strong> <br><img src="{{asset('products/'.$product->image)}}" height=200 width=200>
                    </div>
                    @endif
                @if($product->optional_image)
                        <div class="col-md-6">
                            <strong><strong>Product Image Two (optional) :</strong><br> <img src="{{asset('products/'.$product->optional_image)}}" height="200" width="200">
                        </div>
                @endif
            </div>
                        ...
                        <tbody>
                            @foreach($product->sizes as $key=>$size)
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td>{{$size->title}}</td>
                                    <td>{{$size->stocks->sum('quantity')}}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
